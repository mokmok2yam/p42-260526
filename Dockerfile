...
# 1️⃣ Build Stage
# =========================
# GraalVM JDK 24 기반 Gradle 이미지 (빌드 전용)
FROM gradle:jdk24-graal AS builder

WORKDIR /app

# 래퍼(gradlew) 파일과 폴더, 그리고 빌드 설정 파일을 먼저 복사합니다.
COPY gradlew ./
COPY gradle ./gradle
COPY build.gradle.kts settings.gradle.kts ./

# 호스트 OS(예: 윈도우) 환경에 의해 실행 권한이 유실되는 것을 방지하기 위해 권한을 강제로 부여합니다.
RUN chmod +x ./gradlew

# 의존성 파일만 먼저 다운로드하여 Docker 레이어 캐시 활용
